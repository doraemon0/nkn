...
func (ps *ProposerService) ProduceNewBlock() {
	...
	ownWeight, _ := ledger.DefaultLedger.Store.GetVotingWeight(Uint160{})
	ownNodeID := ps.localNode.GetID()
	votingPool.AddToReceivePool(votingHeight, ownNodeID, ownWeight, blockHash)
	log.Info("produce new block and add a vote for block: ", BytesToHexString(blockHash.ToArrayReverse()))
}
